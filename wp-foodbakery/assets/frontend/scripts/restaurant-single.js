...
$(document).on('click', '.menu-order-confirm', function () {
    ...
    if (!_this.hasClass('is-disabled')) {
        ...
        $.ajax({
            url: ajax_url,
            method: "POST",
            data: '_pay_method=' + pay_method + '&_rid=' + rid + '&order_subtotal_price=' + order_subtotal_price + '&order_vat_percent=' + order_vat_percent + '&order_vat_cal_price=' + order_vat_cal_price + '&action=foodbakery_restaurant_order_confirm',
            dataType: "json"
        }).done(function (response) {
